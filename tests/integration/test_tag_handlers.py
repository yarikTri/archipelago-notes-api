...
def test_create_and_link_tag(api_client, test_note):
    ...
    tags = response.json()
    assert any(tag["tag_id"] == tag_id for tag in tags), (
        "Tag should be linked to the note"
    )

    # Test creating a tag with same name for the same note (should fail)
    response = api_client.post(f"{api_client.base_url}/api/tags/create", json=tag_data)
    assert response.status_code == 409, (
        "Creating a tag with the same name for the same note should fail"
    )
...
def test_delete_tag_invalid_request(api_client):
    ...
    response = api_client.post(f"{api_client.base_url}/api/tags/delete", json={})
    assert response.status_code == 400

    # Test with invalid tag_id format
    response = api_client.post(
        f"{api_client.base_url}/api/tags/delete", json={"tag_id": "invalid-uuid"}
    )
    assert response.status_code == 400
...
def test_delete_tag_with_multiple_relations(api_client, root_dir):
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/create", json=second_tag_data
    )
    assert response.status_code == 201
    second_tag_id = response.json()["tag_id"]
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/delete", json={"tag_id": tag_id}
    )
    assert response.status_code == 200
...
def test_delete_tag_cascade_effects(api_client, root_dir):
    ...
    # Delete tag1
    response = api_client.post(
        f"{api_client.base_url}/api/tags/delete", json={"tag_id": tag1_id}
    )
    assert response.status_code == 200
...
def test_tag_isolation_between_users(
    ...
    assert first_user_tag_id != second_user_tag_id, (
        "Tags with same name for different users should have different IDs"
    )
...
def test_tag_deletion_isolation(
    ...
    response = second_user_client.post(
        f"{second_user_client.base_url}/api/tags/delete",
    ...
    assert any(tag["tag_id"] == first_user_tag_id for tag in tags), (
        "First user's tag should still exist"
    )
...
def test_link_existing_tag(api_client, test_tag, root_dir):
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/{test_tag}/link/{note2_id}"
    )
    assert response.status_code == 201, response.text
    ...
    assert response.status_code == 409, (
        "Linking the same tag to the same note again should fail"
    )
...
def test_tag_with_same_name_for_different_users(
    ...
    assert any(tag["name"] == tag_name for tag in tags), (
        "First user should see their tag"
    )

    # Verify second user can see their tag
    response = second_user_client.get(
        f"{second_user_client.base_url}/api/tags/note/{second_user_note}"
    )
    assert response.status_code == 200
    tags = response.json()
    assert any(tag["name"] == tag_name for tag in tags), (
...
def test_tag_operations_after_user_switch(
    api_client,
    second_user_client,
    test_tag,
    second_user_tag,
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/delete", json={"tag_id": temp_tag_id}
    )
    assert response.status_code == 200
    ...
    assert response.status_code == 200
    tags = response.json()
    assert any(tag["tag_id"] == temp_tag_id_2 for tag in tags), (
        "Second user's tag should still exist"
    )

    # Second user deletes their temporary tag
    response = second_user_client.post(
        f"{second_user_client.base_url}/api/tags/delete", json={"tag_id": temp_tag_id_2}
    )
    assert response.status_code == 200
...
def test_user_cannot_view_tags_from_other_users_notes(
    api_client, second_user_client, test_note, second_user_note
):
...
def test_user_cannot_view_linked_tags_from_other_users_tags(
    api_client, second_user_client, test_note, second_user_note
):
    ...
    response = second_user_client.post(
        f"{second_user_client.base_url}/api/tags/create", json=tag1_data
    )
    assert response.status_code == 201
    ...
    response = second_user_client.post(
        f"{second_user_client.base_url}/api/tags/create", json=tag2_data
    )
    assert response.status_code == 201
    ...
    response = second_user_client.post(
        f"{second_user_client.base_url}/api/tags/link", json=link_data
    )
    assert response.status_code == 200
    ...
    response = second_user_client.get(
        f"{second_user_client.base_url}/api/tags/{second_user_tag1_id}/linked"
    )
    assert response.status_code == 200
    linked_tags = response.json()
    assert len(linked_tags) == 1
...
def test_tag_unlinking_isolation_between_users(
    api_client, second_user_client, test_note, second_user_note
):
...
def test_tag_same_name_isolation_between_users(
    api_client, second_user_client, test_note, second_user_note
):
    ...
    assert first_user_tags[0]["tag_id"] == first_user_tag_id, (
        "First user should see their own tag"
    )
    assert first_user_tags[0]["name"] == shared_tag_name, (
        "First user's tag should have the correct name"
    )
    ...
    assert second_user_tags[0]["tag_id"] == second_user_tag_id, (
        "Second user should see their own tag"
    )
    assert second_user_tags[0]["name"] == shared_tag_name, (
        "Second user's tag should have the correct name"
    )


def test_tag_same_name_deletion_isolation(
    api_client, second_user_client, test_note, second_user_note
):
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/delete", json={"tag_id": first_user_tag_id}
    )
    ...
    assert second_user_tags[0]["tag_id"] == second_user_tag_id, (
        "Second user's tag ID should be unchanged"
    )
    assert second_user_tags[0]["name"] == shared_tag_name, (
        "Second user's tag name should be unchanged"
    )
...
def test_suggest_tags(api_client):
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/suggest", json={"text": test_text}
    )
    assert response.status_code == 200, response.text
    ...
    for tag in tags:
        ...
        assert tag.isalnum() or all(c.isalnum() or c in "-_" for c in tag), (
            "Tag should contain only alphanumeric characters, hyphens, or underscores"
        )
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/suggest", json={"text": ""}
    )
    assert response.status_code == 400, "Should reject empty text"

    # Test with missing text field
    response = api_client.post(f"{api_client.base_url}/api/tags/suggest", json={})
    assert response.status_code == 400, "Should reject missing text field"

    # Test with very long text
    long_text = "word " * 1000  # 5000 characters
    response = api_client.post(
        f"{api_client.base_url}/api/tags/suggest", json={"text": long_text}
    )
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/suggest", json={"text": special_text}
    )
    ...
    tags = response_data["tags"]
    assert len(tags) > 0, (
        "Should return at least one tag for text with special characters"
    )
    ...
    response = api_client.post(
        f"{api_client.base_url}/api/tags/suggest", json={"text": russian_text}
    )
